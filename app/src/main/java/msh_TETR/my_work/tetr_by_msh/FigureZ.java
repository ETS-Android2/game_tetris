...
public final class FigureZ extends TetrFigure {

    //X - down angle of figureI at axis OX
    //Y - down angle of figureI at axis OY


    public FigureZ(FigureDirection direction, int color){
        ...
        switch (direction)
        {
            case Top:
                if(X == 0 || FieldActivity.gameField.mapOfField.get(Y).get(X - 1))
                {
                    ...
                    {
                        if(X - 2 >= 0 && !FieldActivity.gameField.mapOfField.get(Y + 1).get(X - 1) && !FieldActivity.gameField.mapOfField.get(Y).get(X - 1) && !FieldActivity.gameField.mapOfField.get(Y).get(X - 2))
                            forTurn(FigureDirection.Left, -1, 1);
                    }
                    else
